...
if (stage) {
  mountKSpace(stage).catch(error => {
    console.error("Cepha concept renderer failed", error);
    ...
    if (status) {
      status.textContent = document.body.classList.contains("lang-ar")
        ? "تعذر تحميل عقد قالب Cepha؛ لم تُعرض بيانات بديلة."
        : "The Cepha template contract could not be loaded; no substitute data was shown.";
    }
  });
}
...
async function mountKSpace(root) {
  ...
  root.dataset.state = "concept-template";
  updateStaticStatus();
  populateSelect();
  selectItem(
    vectorNodes[0]?.key ||
  ...
  function updateStaticStatus() {
    ...
    if (elements.status) {
      elements.status.textContent = count > 0
        ? text(
          `تم تحميل ${count.toLocaleString("ar-SA")} عقدة من عينة قالب حتمية محفوظة (${evidence.nodeGraph.runtimeId || "runtime-id unavailable"}).`,
          `${count.toLocaleString("en")} nodes loaded from a preserved deterministic template sample (${evidence.nodeGraph.runtimeId || "runtime id unavailable"}).`
        )
        : text(
          "قالب Cepha يعمل الآن على مراسي العرض المتاحة؛ ولا تُختلق عقد بديلة.",
          "The Cepha template is live on the available presentation anchors; no substitute nodes are invented."
        );
    }
    if (elements.count) {
      elements.count.textContent = count > 0
        ? text(`${count} عقدة قالب`, `${count} template nodes`)
        : text("عقد القالب: غير متاحة", "Template nodes: unavailable");
    }
  }
  ...
  function selectItem(key) {
    ...
    if (elements.type) {
      elements.type.textContent = item.type === "node"
        ? text("عقدة عينة تصورية", "Concept-sample node")
        : text(
          item.unavailable ? "مرساة حالة غير متاحة" : "مرساة قالب تصورية",
          item.unavailable ? "Unavailable-state anchor" : "Concept-template anchor"
        );
    }
    if (item.type === "node") {
      ...
      if (elements.proofState) elements.proofState.textContent = "SAMPLE —";
    }
  ...
  function drawCanvasLegend(width, height) {
    ...
    context.fillText(
      vectorNodes.length
        ? "CEPHA CONCEPT REPLAY · SAMPLE ORDER · K_c_n_s"
        : "CEPHA CONCEPT RENDER · TEMPLATE ANCHORS",
      28,
      39
    );
    ...
    context.fillText(
      vectorNodes.length
        ? "gold = sample-order replay"
        : "gold = template replay cursor",
    ...
    try {
      ...
      if (elements.verification) {
        elements.verification.textContent = valid
          ? text(
            "نجح: مسار Merkle وانتقال Hamming يطابقان عقد العينة المحفوظ.",
            "Passed: the Merkle path and Hamming transition match the preserved sample contract."
          )
          : text(
            "فشل تحقق العينة؛ لا تُعامل عقدة القالب بوصفها سليمة.",
            "Sample verification failed; do not treat this template node as valid."
          );
      }
      if (elements.proofState) {
        elements.proofState.textContent = valid ? "SAMPLE OK" : "SAMPLE FAIL";
      }
    } catch (error) {
      ...
      if (elements.proofState) elements.proofState.textContent = "SAMPLE ERROR";
    } finally {
      elements.verify?.removeAttribute("disabled");
    }
  }
